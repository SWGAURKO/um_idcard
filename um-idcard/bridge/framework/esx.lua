...
local function GetMetaLicense(src, itemTable)
    ...
    if type(itemTable) == 'string' then itemTable = { itemTable } end
    if type(itemTable) == 'table' then
        for _, v in pairs(itemTable) do
            return {
                cardtype = v,
...
function CreateRegisterItem(k)
    if hasQs then
        ESX.RegisterUsableItem(k, function(source, item)
            ...
            if not metadata.cardtype and item.name then metadata.cardtype = item.name end
            TriggerEvent('um-idcard:server:sendData', source, metadata)
        end)
        return
    end
